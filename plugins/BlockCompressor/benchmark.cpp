...
void measure_compression_time_cmbf(BlockCompressor& compressor, const std::string& matrix_path, const std::string& config_path, unsigned short header)
{
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    BlockCompressor::compress_cmbf(compressor, matrix_path, "test_compression", config_path, header);
    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
...
int benchCMBF(BlockCompressor& compressor, BlockDecompressor& bd, std::ifstream& matrix, const std::string& matrix_path, const std::string& config_path, std::uint64_t minimum_hash, std::uint64_t maximum_hash, unsigned short header)
{
    ...
    measure_compression_time_cmbf(compressor, matrix_path, config_path, header);
...
int main(int argc, char ** argv)
{
    ...
    else if(path.extension() == ".cmbf")
    {
        if(compressor == "LZMA")
        {
            ...
            benchCMBF(bc, bd, matrix, matrix_path, config_path, minimum_hash, maximum_hash, header);
        }
